    ...
    Node("lslidar_driver_node", options), diagnostics(this)
{
	...
	if (!this->initialize()) 
		RCLCPP_ERROR(this->get_logger(), "初始化雷达驱动失败...");
	else
		RCLCPP_INFO(this->get_logger(), "初始化雷达驱动成功...");
}
...
bool LslidarDriver::initialize() {
    if (!loadParameters()) {
        RCLCPP_ERROR(this->get_logger(), "不能加载所需的ros参数...");
        return false;
    }
    if(interface_selection == "net"){
        if (!createRosIO()) {
            RCLCPP_ERROR(this->get_logger(), "不能创建所有的ros IO...");
            return false;
        }
    }
    else
    ...
    // RCLCPP_INFO(this->get_logger(), "初始化雷达正常");
    return true;
}
...
void LslidarDriver::recvThread_crc(int &count,int &link_time)
{
	...
	if(link_time > 150)
	{
		...
		if(ret < 0)
		{
            RCLCPP_ERROR(this->get_logger(),"雷达串口打开失败");
			usleep(200000);
		}
		link_time = 0;
	}
